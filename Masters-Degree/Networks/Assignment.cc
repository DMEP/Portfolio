...
NS_LOG_COMPONENT_DEFINE ("AssignmentT2"); // RW RD
...
{
	ofstream outfile;
	outfile.open("AssignmentT2_general.txt", ios::app); // RW RD
	Ptr<Packet> packet;
	Address from;
...
int main (int argc, char *argv[])
{
	...
	long double interval = 5; // Used in DataRate 1, 3, 5 
	uint32_t numNodes = 250; //Number node; 50, 150, 250
	uint32_t speed = 20; //Node speed; 20
	...
	outfile << "Simulator Parameters" << endl
			<< "Routing Protocol : DSDV" << endl // AODV, DSDV
			<< "Number of Nodes: " << numNodes << endl
			<< "Speed: " << speed << " m/s" << endl
			<< "Each node sends: " << numPackets << endl
			<< "Data rate: " << ((packetSize/interval)*8)/1024 << " Mbps" << endl;

	outfile2.open("AssignmentT2_results.txt", ios::app); // RW RD
...
/*
	...
	*/
	...
	internet.SetRoutingHelper(dsdv); //aodv, dsdv
	...
	wifiPhy.EnableAsciiAll (ascii.CreateFileStream ("AssignmentT2.tr")); // dsdv, aodv
	Simulator::Run ();
	Simulator::Destroy();
	...
	outfile2 << "----Simulator Results----" << endl <<
				...
				"Packet %: " << double(received_packets*100)/(numPackets*(numNodes -	1)) << "%." << endl << 
